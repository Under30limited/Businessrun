...
import { RenewalAlert, TeamAccessBlockedOverlay } from './SubscriptionAlerts';
...
// ─────────────────────────────────────────────────────────────────
const ADVISOR_GREETING = 'TBR Strategic AI active. Share your current business progress or expansion plans. How can I help you scale today?';

function AdvisorChat({ initialPrompt, onPromptConsumed, language, profile, cfoEntries, inventoryItems, sales }) {
  const [messages, setMessages] = useState([
    { role: 'assistant', content: ADVISOR_GREETING },
  ]);
  ...
  // Whether `messages` currently holds a real persisted thread loaded
  // from the server (vs just the local greeting placeholder) — affects
  // both how `history` is sliced when sending (don't drop index 0 if
  // it's a genuine past message) and what "clear" needs to reset.
  const [hasPersistedHistory, setHasPersistedHistory] = useState(false);

  // Tracks whether a message has been sent yet since this component
  // mounted (i.e. since the Advisor tab was last opened) — independent
  // of messages.length, which may already be >1 right after hydrating
  // real history. Drives `refreshContext` (BASE DATA re-injection) and
  // the inventory/sales snapshot baseline — both should happen once
  // per tab-open, not once per empty-thread.
  const sentThisSessionRef = useRef(false);

  // ── Hydrate persisted chat on mount ──────────────────────────────
  // One continuous thread per business (see backend) — load it so
  // reopening the Advisor tab resumes where the last conversation
  // left off instead of starting blank. Silently falls back to the
  // local greeting on any failure (network issue, Zero-tier business
  // with nothing stored, member without 'advisor' permission) — the
  // advisor still works, it just won't have history for this load.
  useEffect(() => {
    let cancelled = false;
    (async () => {
      try {
        const res = await fetch('/api/advisor/history', { credentials: 'include' });
        if (!res.ok) return;
  ...
  async function sendMessage(text) {
    ...
    // ── First message this tab-open — mark snapshot baseline ────
    // The server will inject/refresh BASE DATA on this turn (see
    // `refreshContext` below). Record what the inventory and sales
    // look like right now so we can detect changes from here.
    const isFirstMessageThisSession = !sentThisSessionRef.current;
    sentThisSessionRef.current = true;
    if (isFirstMessageThisSession) {
    ...
    let deltaNote = null;
    if (!isFirstMessageThisSession && baseSnapshotRef.current) {
    ...
    setIsLoading(true);
    try {
      // Build history from the conversation — exclude the current pending
      // user message (last entry). Also drop index 0 when it's just the
      // local greeting placeholder (no persisted history loaded) — but
      // KEEP it when it's a genuine hydrated past message, since dropping
      // it there would silently exclude real history from the model.
      ...
      const res  = await fetch('/api/advisor', {
        ...
        body: JSON.stringify({
          ...
          history,
          refreshContext: isFirstMessageThisSession,
  ...
  // Clears both the visible chat and the business's persisted thread
  // server-side — without the DELETE call this would just look cleared
  // and then silently reappear (with everything before it too) the next
  // time the Advisor tab is reopened and history rehydrates.
  async function clearChat() {
    chatInteracted.current      = false;
    sentThisSessionRef.current  = false;
    setHasPersistedHistory(false);
    baseSnapshotRef.current     = null;
    setMessages([{ role: 'assistant', content: 'Chat cleared. How can I assist?' }]);
    try {
      await fetch('/api/advisor/history', { method: 'DELETE', credentials: 'include' });
    } catch {
      // Non-fatal — worst case the old thread resurfaces on next reopen;
      // the visible chat is already cleared for this session either way.
    }
  }
...
export default function RoadmapPage() {
  ...
  const { user, subscription, isAuthenticated, isRestoring, logout } = useAuth();
      ...
      <div className="bg-white border border-zinc-200 rounded-3xl w-full max-w-sm overflow-hidden shadow-2xl">
        ...
        <div className="px-7 pt-6 pb-7">
          ...
          <div className="space-y-2">
            {LANGUAGES.map(lang => {
              const allowedLanguages = subscription?.limits?.languages || ['English'];
              const isLocked = !allowedLanguages.includes(lang.code);
              return (
                <button
                  key={lang.code}
                  onClick={() => isLocked ? navigate('/billing') : selectLanguage(lang.code)}
                  className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl border text-left transition-all group ${
                    isLocked
                      ? 'border-zinc-100 bg-zinc-50 opacity-60'
                      : 'border-zinc-200 hover:border-amber-500/40 hover:bg-amber-500/5'
                  }`}
                >
                  <span className="text-xl">{lang.flag}</span>
                  <div>
                    <p className="text-sm font-black text-zinc-800 group-hover:text-zinc-900">{lang.label}</p>
                    <p className="text-[10px] text-zinc-600">{isLocked ? 'Upgrade to unlock' : lang.code}</p>
                  </div>
                  {isLocked
                    ? <Lock size={13} className="text-zinc-400 ml-auto flex-shrink-0" />
  ...
  return (
    <div className="min-h-screen bg-white text-zinc-900 flex">
      ...
      <RenewalAlert />
      <LowStockBanner onViewInventory={() => openView('inventory')} />
        ...
        <div className="flex-shrink-0 border-t border-zinc-100 p-3 space-y-1">
          ...
          {!isTeamMember && (
            <button
              ...
              title={!sidebarOpen ? 'Billing & Plans' : undefined}
              className={`w-full flex items-center rounded-xl transition border
                ${sidebarOpen ? 'gap-3 px-4 py-3' : 'justify-center px-0 py-3'}
                text-zinc-500 hover:bg-zinc-100 hover:text-zinc-900 border-transparent
              `}
            >
              ...
              {sidebarOpen && <span className="text-[11px] font-black uppercase tracking-widest">Billing & Plans</span>}
            </button>
          )}
          <button
      ...
      <ContactTrigger source="business" />
    </div>
  );
}
