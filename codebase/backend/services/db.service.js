 ...
 *   br-advisorSessions PK: sessionId (String) — value stored here is
 *                      the business's businessUid; one row per
 *                      BUSINESS (one continuous advisor thread, not
 *                      multiple listable sessions).
 *                      { sessionId: businessUid, messages: [{ role,
 *                      content, timestamp }], createdAt, updatedAt }.
 *                      Pruned per-tier by plan.limits.advisorMemoryDays
 *                      on every write (and defensively on read) — see
 *                      getAdvisorHistory/appendAdvisorMessages.
...
const TABLES = {
  ...
  SUBSCRIPTIONS:  'br-subscriptions',
  USAGE_COUNTERS: 'br-usageCounters',
...
async function getUserByEmail(email) {
  ...
  // Deliberately NO Limit here. email-index has no sort key, so when
  // more than one row shares this email — e.g. the real identity AND
  // a not-yet-deleted GYB session doc, which briefly coexist right
  // after signup since session cleanup is fire-and-forget — DynamoDB
  // can return them in ANY order. Limit:1 combined with a
  // FilterExpression is a classic trap: Limit caps how many items are
  // SCANNED before the filter runs, not how many are RETURNED after
  // it — so the one real claimed identity could be skipped entirely
  // if a different row got scanned first, making login intermittently
  // fail to find an account that genuinely exists. Fetching every
  // match for this email (realistically at most 2-3 rows, never many)
  // and choosing in application code is the only reliable approach.
  ...
  // Prefer a genuinely claimed identity (real password set); fall
  // back to any match (used by checkEmail's "exists but maybe
  // unclaimed" case, and by a still-pending invite's identity stub).
  return items.find(i => i.claimed === true) || items[0] || null;
...
// SUBSCRIPTIONS
//
// One row per business. PK: businessUid — a business has exactly one
// subscription at a time (never a history table; Paystack/webhook
// events overwrite this row in place). See config/plans.js for what
// each planId actually includes — this table only stores WHICH plan
// a business is on, never the entitlements themselves.
//
// STATUS LIFECYCLE:
//   'trialing' — on the 30-day free trial of plus1, no payment yet
//   'active'   — a real, currently-paid-for plan
//   'past_due' — payment failed/lapsed; grace handling happens in the
//                enforcement layer (middleware/plan.js), not here
//   'canceled' — explicitly canceled, will drop to the fallback plan
//                at currentPeriodEnd
// ─────────────────────────────────────────────────────────────────

/**
 * createSubscription
 * Starts a business on the standard trial — called once, right after
 * a business is created (GYB step 4) and by the one-time migration
 * script for every business that existed before subscriptions did.
 *
 * billingInterval / paystackEmailToken are added later by
 * updateSubscription() once a real payment happens — see
 * controllers/payments.controller.js. billingInterval ('monthly' |
 * 'annually') drives how far currentPeriodEnd gets extended on
 * renewal; paystackEmailToken (delivered only via the
 * subscription.create webhook, alongside paystackSubscriptionCode) is
 * what a future plan/interval SWITCH uses to cancel this subscription
 * via paystack.disableSubscription() — each interval is a genuinely
 * separate Paystack Plan object, so switching means canceling the old
 * one, not editing it in place.
 *
 * @param {string} businessUid
 * @param {Object} [overrides]  Only used by the migration script to
 *                               set a fresh trialEndsAt for pre-existing
 *                               businesses — new signups never need this.
 */
async function createSubscription(businessUid, overrides = {}) {
  const { TRIAL_PLAN_ID, TRIAL_DURATION_MS } = require('../config/plans');
  const now = nowISO();
  const trialEndsAt = overrides.trialEndsAt || new Date(Date.now() + TRIAL_DURATION_MS).toISOString();
  ...
  const item = {
    ...
    status:                    'trialing',
    trialEndsAt,
    currentPeriodEnd:          trialEndsAt, // trial IS the current period until a real payment happens
    paystackCustomerCode:      null,
    paystackSubscriptionCode:  null,
    paystackEmailToken:        null,        // set once a real payment is made — needed to cancel on a future switch
    billingInterval:           null,        // set once a real payment is made ('monthly' | 'annually')
    currency:                  null,        // set once a real payment is made ('USD' | 'NGN')
    createdAt:                 now,
    updatedAt:                 now,
  };

  await dynamo.send(new PutCommand({ TableName: TABLES.SUBSCRIPTIONS, Item: item }));
  console.log(`[DB] Subscription created: ${businessUid} → trialing ${TRIAL_PLAN_ID} until ${trialEndsAt}`);
  return item;
}
 ...
 */
async function getSubscription(businessUid) {
  const result = await dynamo.send(new GetCommand({
...
/**
 * updateSubscription
 * Patches arbitrary fields — used for plan changes, status
 * transitions, and recording Paystack customer/subscription codes.
 *
 * @param {string} businessUid
 * @param {Object} updates
 */
async function updateSubscription(businessUid, updates) {
  const names  = { '#updatedAt': 'updatedAt' };
  const values = { ':updatedAt': nowISO() };
  const sets   = ['#updatedAt = :updatedAt'];
 ...
 * Reverse lookup — used by the payment webhook handler for renewal
 * charges, which may not reliably carry the metadata we set on the
 * ORIGINAL initializing transaction. Requires the
 * paystackCustomerCode-index GSI.
 *
 * @param {string} customerCode
 * @returns {Promise<Object|null>}
 */
async function getSubscriptionByPaystackCustomerCode(customerCode) {
  const result = await dynamo.send(new QueryCommand({
    TableName:                 TABLES.SUBSCRIPTIONS,
    IndexName:                 'paystackCustomerCode-index',
    KeyConditionExpression:    '#code = :code',
    ExpressionAttributeNames:  { '#code': 'paystackCustomerCode' },
    ExpressionAttributeValues: { ':code': customerCode },
    Limit: 1,
  }));
  return result.Items && result.Items.length > 0 ? result.Items[0] : null;
}
...
// USAGE COUNTERS
//
// Generic per-business, per-metric, per-period counters — currently
// used for the AI Advisor's daily question limit. PK: businessUid,
// SK: metricPeriod (e.g. "advisorQuestions:2026-08-06"). Deliberately
// generic (metric name is just a string) so a future usage-based
// limit can reuse this same table rather than needing its own.
//
// TTL is set ~2 days past the period so old counters clean themselves
// up automatically — this is housekeeping only, NOT the enforcement
// mechanism (enforcement reads and compares the count directly; DynamoDB's
// TTL sweep can lag by up to 48 hours per AWS's own docs, so it must
// never be relied on for correctness, only for eventually freeing storage).
// ─────────────────────────────────────────────────────────────────

/**
 * getUsageCount
 * @param {string} businessUid
 * @param {string} metric   e.g. 'advisorQuestions'
 * @param {string} period   e.g. '2026-08-06' (UTC date)
 * @returns {Promise<number>}
 */
async function getUsageCount(businessUid, metric, period) {
  const result = await dynamo.send(new GetCommand({
    TableName: TABLES.USAGE_COUNTERS,
    Key:       { businessUid, metricPeriod: `${metric}:${period}` },
  }));
 ...
 * Atomically increments the counter for this business/metric/period,
 * creating it at 1 if it doesn't exist yet. Call this AFTER the
 * action it's counting has actually succeeded — don't count a request
 * that failed partway through.
 *
 * @param {string} businessUid
 * @param {string} metric
 * @param {string} period
 * @returns {Promise<number>}  the new count, in case a caller wants it
 */
async function incrementUsageCounter(businessUid, metric, period) {
  const metricPeriod = `${metric}:${period}`;
  const ttl = Math.floor(Date.now() / 1000) + (2 * 24 * 60 * 60); // ~2 days past period, cleanup only

  const result = await dynamo.send(new UpdateCommand({
    TableName:                 TABLES.USAGE_COUNTERS,
    Key:                       { businessUid, metricPeriod },
...
// ADVISOR SESSIONS
// ─────────────────────────────────────────────────────────────────
// ONE continuous thread per business — not multiple listable
// sessions — so br-advisorSessions is keyed directly by businessUid
// (stored in the `sessionId` field/PK, same column that existed
// before; only the meaning of "one row" changed, not the schema).
// No GSI needed: every access is already a direct key lookup by the
// caller's own businessUid, same access pattern as every other
// per-business table (br-inventory, br-cfoEntries, etc).
//
// Each message is stored as { role, content, timestamp } — the
// timestamp (absent before) is what makes the 30-day pruning for
// BusinessRun +1 possible.

/**
 * Drops messages older than `memoryDays` (if finite). Pass null for
 * unlimited tiers to skip pruning entirely — a no-op, not a query.
 */
function pruneMessagesByAge(messages, memoryDays) {
  if (memoryDays === null || memoryDays === undefined) return messages;
...
/**
 * getAdvisorHistory
 * Returns the business's stored thread, already pruned to
 * `memoryDays`. Returns [] if nothing stored yet.
 *
 * @param {string} businessUid
 * @param {number|null} memoryDays  plan.limits.advisorMemoryDays
 */
async function getAdvisorHistory(businessUid, memoryDays) {
  const result = await dynamo.send(new GetCommand({
    TableName: TABLES.ADVISOR,
    Key:       { sessionId: businessUid },
  }));
  if (!result.Item) return [];
  return pruneMessagesByAge(result.Item.messages, memoryDays);
}

/**
 * appendAdvisorMessages
 * Appends the latest user+assistant turn to the business's thread,
 * pruning anything past `memoryDays` in the same write. Callers on
 * the Zero tier (memoryDays === 0) should never call this at all —
 * see the controller, which skips persistence entirely rather than
 * writing-then-immediately-pruning-to-empty here.
 *
 * @param {string} businessUid
 * @param {Array}  newMessages   the full existing thread (already
 *                                fetched by the caller) plus the new
 *                                turn appended, each with a timestamp
 * @param {number|null} memoryDays
 */
async function appendAdvisorMessages(businessUid, newMessages, memoryDays) {
  const now    = nowISO();
  const pruned = pruneMessagesByAge(newMessages, memoryDays);
  ...
  if (existing.Item) {
    const { UpdateExpression, ExpressionAttributeNames, ExpressionAttributeValues } =
      buildUpdateExpr({ messages: pruned, updatedAt: now });
    await dynamo.send(new UpdateCommand({
      TableName: TABLES.ADVISOR,
      Key:       { sessionId: businessUid },
      UpdateExpression,
    ...
    await dynamo.send(new PutCommand({
      TableName: TABLES.ADVISOR,
      Item: { sessionId: businessUid, messages: pruned, createdAt: now, updatedAt: now },
    }));
  }
}
...
/**
 * clearAdvisorHistory
 * Backs the dashboard's "Clear chat" button — deletes the business's
 * entire stored thread. Safe to call even if nothing was ever stored.
 *
 * @param {string} businessUid
 */
async function clearAdvisorHistory(businessUid) {
  await dynamo.send(new DeleteCommand({
    TableName: TABLES.ADVISOR,
    Key:       { sessionId: businessUid },
  }));
...
async function getSales(uid, fromDate, toDate) {
  // Always query via the uid-saleDateISO-index GSI — even with no date
  // filter — so results come back in chronological order (the base
  // table's sort key is saleId, a random UUID, which sorts results by
  // creation-id order, not by date).
  const params = {
    ...
    KeyConditionExpression:    '#uid = :uid' +
      ...
      : toDate            ? ' AND #saleDateISO <= :to'
                          : ''),
    ExpressionAttributeNames:  {
      '#uid': 'uid',
      ...(fromDate || toDate ? { '#saleDateISO': 'saleDateISO' } : {}),
    },
    ExpressionAttributeValues: {
      ':uid':  uid,
...
async function getItemSaleHistory(uid, itemId) {
  const result = await dynamo.send(new QueryCommand({
    ...
    ExpressionAttributeNames:  { '#uid': 'uid' },
    ExpressionAttributeValues: { ':uid': uid },
    ScanIndexForward: false, // newest first — same convention as getSales
  }));
