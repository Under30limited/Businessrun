...
/**
 * Generates a 6-digit OTP, stores it in the OTP store, and emails it.
 *
 * IMPORTANT: Always returns success regardless of whether the email
 * is registered — prevents email enumeration attacks.
 * The actual email is only sent if the user exists in the database.
 *
 * Body: { email }
 ...
 * user's hashed password via db.service (DynamoDB).
 *
 * Authentication in this system uses the DynamoDB users table + bcrypt
 * exclusively. Firebase Auth is not involved — no createUser() is ever
 * called during signup, so no updateUser() is called here either.
 *
...
const resetPassword = asyncHandler(async (req, res) => {
  ...
  // Update password via the service layer (DynamoDB) — this is the
  // only auth store we use. Firebase Auth is NOT used for authentication
  // in this system — login goes through db.service + bcrypt entirely, so
  // no Firebase Auth update is needed or possible (users are never
  // created in Firebase Auth).
  await firebaseService.updateUserPassword(user.uid, hashedPassword);
