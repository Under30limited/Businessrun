...
export default function RoadmapPage() {
  ...
  // the sidebar, so visibleNavItems filtering never touched it. Each
  // sub-item maps to the permission for the data it reports on: a
  // Sales Report needs 'sales', an Inventory Report needs 'inventory'.
  // Owners always see both, unchanged.
  const GENERATE_ITEMS = [
    { id: 'generate-sales',     label: 'Sales Report',     icon: <TrendingUp size={13} />, requires: 'sales' },
    { id: 'generate-inventory', label: 'Inventory Report', icon: <Package size={13} />,     requires: 'inventory' },
  ];
  const visibleGenerateItems = GENERATE_ITEMS.filter(item => {
    if (!isTeamMember) return true;
